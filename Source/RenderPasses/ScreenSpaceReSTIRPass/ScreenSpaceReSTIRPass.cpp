...
namespace
{
    ...
    const Falcor::ChannelList kInputChannels =
    {
        ...
        { "GIColor", "gGIColor", "Indirect Illumination", true /* optional */, ResourceFormat::RGBA32Float },
    };
...
void ScreenSpaceReSTIRPass::execute(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    const auto& pInColor = renderData["GIColor"]->asTexture();
    ...
    for (int i = 0; i < mpScreenSpaceReSTIR.size(); i++)
    {
        ...
        finalShading(pRenderContext, pVBuffer, pInColor, renderData, i);
...
void ScreenSpaceReSTIRPass::finalShading(RenderContext* pRenderContext, const Texture::SharedPtr& pVBuffer, const Texture::SharedPtr& pInColor, const RenderData& renderData, int instanceID)
{
    assert(!mpScreenSpaceReSTIR.empty() && mpScreenSpaceReSTIR[instanceID]);
